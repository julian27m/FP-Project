...
define
    ...
    class Program
        ...
        meth execute()
            ...
            local CallBackList FunctionList in
                {self getCallBacks(CallBackList)}
                for CallBack in CallBackList do
                    {Show "This is a call back <" # CallBack # ">"}
                    {FullFillFromCallBack {Parse CallBack}}
                end
            
                {self getFunctions(FunctionList)}
                for FunctionDef in FunctionList do
                    {Show "Parsing function definition: " # FunctionDef}
                    {Parse FunctionDef}
                end
            end
        end
